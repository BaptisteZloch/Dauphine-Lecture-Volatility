...
class DataLoader(ABC):
    _EXTENSION_TO_LOADER = {"parquet": pd.read_parquet, "csv": pd.read_csv, "xlsx": pd.read_excel}
    ...
    def load_data(
        ...
        check_is_true(start_date >= min_date and end_date <= max_date, f"Data is only available between {min_date.date()} and {max_date.date()}")
        extension = file_path.split(".")[-1]
        check_is_true(extension in cls._EXTENSION_TO_LOADER, f"Unsupported file extension: {extension}")
        logging.info("Reading between %s %s from %s with %s", start_date, end_date, file_path, load_kwargs)
        ...
        df_processed = cls._add_extra_fields(cls._process_loaded_data(df, **(process_kwargs or {})), **(extra_fields_kwargs or {}))
